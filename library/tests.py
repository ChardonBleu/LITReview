...
def current_user(db) -> User:
    return User.objects.create_user(username='moi',
...
def other_user(db) -> User:
    return User.objects.create_user(username='lautre',
                                    password='password_autre_test')


@pytest.fixture
def third_user(db) -> User:
    return User.objects.create_user(username='letroisieme',
                                    password='password_autre_test')


@pytest.fixture
def one_ticket(db, current_user: User) -> Ticket:
    return Ticket.objects.create(title='Mon livre préféré',
                                 user=current_user)
...
def one_review(db, current_user: User, one_ticket: Ticket) -> Review:
    return Review.objects.create(headline='Très bon livre',
                                 user=current_user, rating=5,
                                 ticket=one_ticket)
...
def other_review(db, other_user: User, other_ticket: Ticket) -> Review:
    return Review.objects.create(headline='Très bon livre',
                                 user=other_user, rating=5,
                                 ticket=other_ticket)


@pytest.fixture
def user_follows_other_user(db, current_user: User,
                            other_user: User) -> UserFollows:
    return UserFollows.objects.create(user=current_user,
...
def test_str_review(other_user: User, one_ticket: Ticket) -> None:
    Review.objects.create(ticket=one_ticket, rating=3,
                          headline='Bon livre',
                          user=other_user)
    review = Review.objects.get(id=1)
    assert str(review) == 'Bon livre - by lautre - ' +\
        'related to ticket Mon livre préféré'
...
def test_review_creation_invalid_response(connect_client: Client,
                                          one_ticket: Ticket) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:review_creation'),
                                     data={'title': '',
                                           'description': 'quel beau livre',
...
def test_review_for_ticket_view(connect_client: Client,
                                one_ticket: Ticket) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.get(reverse('library:review_ticket',
                                            args=[1]),
                                    data={})
    assert response.status_code == 200


def test_review_for_ticket_creation(connect_client: Client,
                                    one_ticket: Ticket) -> None:
    review_count = Review.objects.count()
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:review_ticket',
                                             args=[1]),
...
def test_review_for_ticket_invalid_response(connect_client: Client,
                                            one_ticket: Ticket) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:review_ticket',
                                             args=[1]),
...
def test_modify_ticket_view(connect_client: Client,
                            one_ticket: Ticket) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.get(reverse('library:modify_ticket',
                                            args=[1]),
                                    data={})
    assert response.status_code == 200
...
def test_ticket_update(connect_client: Client, one_ticket: Ticket) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:modify_ticket',
                                             args=[1]),
...
def test_ticket_update_unauthorized(connect_client: Client,
                                    other_ticket: Ticket) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:modify_ticket',
                                             args=[1]),
                                     data={'title': 'new book update',
                                           'description': 'quel beau livre',
                                           'image': 'logo.jpg'})
    assert response.content == b"Vous ne pouvez modifier un ticket dont " +\
        b"vous n'\xc3\xaates pas l'auteur.<a href='../../../posts/'>Retour</a>"


def test_ticket_update_response_invalid(connect_client: Client,
                                        one_ticket: Ticket) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:modify_ticket',
                                             args=[1]),
...
def test_delete_ticket_view(connect_client: Client,
                            one_ticket: Ticket) -> None:
    connected_client, connected_user = connect_client
    Ticket.objects.create(title='livre à supprimer',
                          description='description', user=connected_user)
    response = connected_client.delete(reverse('library:delete_ticket',
                                               args=[2]))
    assert response.status_code == 302


def test_delete_ticket_error(connect_client: Client, one_ticket: Ticket,
                             other_user: User) -> None:
    connected_client, connected_user = connect_client
    Ticket.objects.create(title='livre à supprimer', description='description',
                          user=other_user)
    response = connected_client.delete(reverse('library:delete_ticket',
                                               args=[2]))
    assert response.content == b"Vous ne pouvez supprimer un ticket dont " +\
        b"vous n'\xc3\xaates pas l'auteur.<a href='../../../posts/'>Retour</a>"


def test_deldete_review_view(connect_client: Client, one_ticket: Ticket,
                             one_review: Review) -> None:
    connected_client, connected_user = connect_client
    Review.objects.create(headline='critique à supprimer',
                          ...
                          user=connected_user,
                          ticket=one_ticket)
    response = connected_client.delete(reverse('library:delete_review',
                                               args=[2]))
    assert response.status_code == 302


def test_deldete_review_error(connect_client: Client, one_ticket: Ticket,
                              one_review: Review, other_user: User) -> None:
    connected_client, connected_user = connect_client
    Review.objects.create(headline='critique à supprimer',
                          body='description',
                          rating=4,
    ...
    response = connected_client.delete(reverse('library:delete_review',
                                               args=[2]))
    assert response.content == b"Vous ne pouvez supprimer une critique " +\
        b"dont vous n'\xc3\xaates pas l'auteur.<br>" +\
        b"<a href='../../../posts/'>Retour</a>"


def test_modify_review_view(connect_client: Client,
                            one_review: Review) -> None:
    connected_client, connected_user = connect_client
...
def test_review_update(connect_client: Client, one_review: Review) -> None:
    ...
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:modify_review',
                                             args=[1]),
                                     data={'headline': 'Trés trés bon livre',
                                           'body': '',
                                           'rating': '3'})
...
def test_review_update_unauthorized(connect_client: Client,
                                    other_review: Review) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:modify_review',
                                             args=[1]),
                                     data={'title': 'new book update',
                                           'description': 'quel beau livre',
                                           'image': 'logo.jpg'})
    assert response.content == b"Vous ne pouvez modifier une critique dont " +\
        b"vous n'\xc3\xaates pas l'auteur.<a href='../../../posts/'>Retour</a>"


def test_review_update_response_invalid(connect_client: Client,
                                        one_review: Review) -> None:
    connected_client, connected_user = connect_client
    response = connected_client.post(reverse('library:modify_review',
                                             args=[1]),
...
def test_userfollows_creation(connect_client: Client, other_user: User,
...
def test_userfollows_error_yetfollowed(connect_client: Client,
    ...
    response = connected_client.post(reverse('library:following'),
                                     data={'followed_user': 2,
                                           'user': connected_user})
    assert response.content == b"Vous suivez d\xc3\xa9j\xc3\xa0 cet " +\
        b"utilisateur.<br><a href='../../../following/'>Retour</a>"


def test_userfollows_creation_invalid(connect_client: Client,
                                      other_user: User,
                                      third_user: User) -> None:
    connected_client, connected_user = connect_client
    userfollows_count = UserFollows.objects.count()
...
def test_delete_userfollows_view(connect_client: Client,
                                 third_user: User) -> None:
    connected_client, connected_user = connect_client
    ...
    response = connected_client.delete(reverse('library:delete_subscription',
                                               args=[1]))
...
def test_delete_userfollows_error(connect_client: Client, third_user: User,
                                  other_user: User) -> None:
    connected_client, connected_user = connect_client
    UserFollows.objects.create(followed_user=third_user, user=other_user)
    response = connected_client.delete(reverse('library:delete_subscription',
                                               args=[1]))
    assert response.content == b"Vous ne pouvez pas supprimer" +\
        b" un utilisateur que vous ne suivez pas.<br>" +\
